...
const baseLinks = [["Accueil","/"],["Formations","/formations"],["Abonnements","/abonnements"],["Nos sites","/sites"],["Nos services","/services"],["Nous contacter","/contact"]];
...
export default function Header(){
 ...
 useEffect(()=>{
   ...
   fetch("/api/site-pages").then(r=>r.json()).then(j=>{
     if(active && Array.isArray(j.pages)) setLinks([...baseLinks,...j.pages.map(p=>[p.nav_label||p.title,`/${p.slug}`])]);
   }).catch(()=>{});
