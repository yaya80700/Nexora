...
import { Menu, Sparkles, X, UserRound, LogOut, LoaderCircle, MessageCircle } from "lucide-react";
import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import Logo from "./Logo";
import { createClient } from "../../lib/supabase/client";

const baseLinks = [["Accueil","/"],["Formations","/formations"],["Abonnements","/abonnements"],["Nos projets","/projets"],["Nos sites","/sites"],["Nos services","/services"],["Nous contacter","/contact"]];
...
export default function Header(){
 ...
 useEffect(()=>{
   ...
   supabase.auth.getUser().then(({data})=>{
     if(data?.user) fetch("/api/demandes/unread").then(r=>r.json()).then(j=>{if(active)setUnread(Number(j.count)||0)}).catch(()=>{});
   }).catch(()=>{});

   const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{
     if(active) {
       setUser(session?.user || null);
       if(session?.user) fetch("/api/demandes/unread").then(r=>r.json()).then(j=>{if(active)setUnread(Number(j.count)||0)}).catch(()=>{});
       else setUnread(0);
     }
     if(active) setLoading(false);
   });
 ...
 return <header className="nav">
  ...
  <div className="navRight">
   {user && <Link className="messagesNav" href="/demandes" onClick={()=>setUnread(0)} title="Mes échanges"><MessageCircle size={15}/>{unread>0&&<span>{unread}</span>}</Link>}
