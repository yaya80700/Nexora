...
export default function AdminControlNav({active="catalog",onPanelChange}){
 const item=(key,label,href)=>{
  ...
  if(onPanelChange) return <button type="button" key={key} className={cls} onClick={()=>onPanelChange(key)}>{label}</button>;
 ...
 return <nav className="controlPanelNav" aria-label="Sections du panneau de contrôle">
  {item("dashboard","📊 Tableau de bord","/admin")}
  {item("catalog","📦 Catalogue","/admin")}
  {item("projects","🏆 Réalisations","/admin")}
  ...
  {item("academy","🎓 Nexora Academy","/admin")}
 </nav>;
}
