...
import { Plus, Pencil, Trash2, Save, ShieldCheck, LogOut, GraduationCap, Wrench, Globe2, X } from "lucide-react";
...
export default function Admin(){
 const [items,setItems]=useState([]); const [tab,setTab]=useState("formation"); const [editing,setEditing]=useState(null); const [form,setForm]=useState(empty); const [error,setError]=useState(""); const [loading,setLoading]=useState(true);
 ...
 return <main className="adminPage"><header className="adminTop"><div><span className="sectionTag">NEXORA ADMIN</span><h1>Panneau de <span>contrôle.</span></h1><p>Gérez le catalogue public sans modifier le code.</p></div><div className="adminActions"><Link href="/admin/users" className="secondary">Utilisateurs</Link><Link href="/admin/demandes" className="secondary">Demandes</Link><Link href="/admin/editor" className="secondary">Éditeur complet</Link><Link href="/" className="secondary">Voir le site</Link><button className="secondary" onClick={logout}><LogOut size={15}/> Déconnexion</button></div></header>
