...
export default function Admin(){
 const [items,setItems]=useState([]); const [subscriptions,setSubscriptions]=useState([]); const [tab,setTab]=useState("formation"); const [panel,setPanel]=useState("dashboard"); const [editing,setEditing]=useState(null); const [adding,setAdding]=useState(false); const [form,setForm]=useState(empty); const [error,setError]=useState(""); const [loading,setLoading]=useState(true);
 ...
 <section className="adminWrap">
  ...
  {panel==="dashboard"?<DashboardPanel />:panel==="editor"?<Editor />:panel==="projects"?<ProjectsAdmin />:panel==="users"?<AdminUsers embedded />:panel==="requests"?<AdminDemandes embedded />:<>
