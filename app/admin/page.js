...
export default function Admin(){
 ...
 <section className="adminWrap">
  ...
  {(editing!==null || adding) && <form className="adminForm" onSubmit={save}><div className="adminFormTitle"><strong>{editing?"Modifier":"Ajouter"} — {labels[tab]}</strong><button type="button" onClick={()=>{setEditing(null);setAdding(false);setForm({...empty,type:tab})}}><X size={17}/></button></div>
   ...
   {tab==="service"&&<><label>Image du service (URL)<input type="url" value={form.image_url||""} onChange={e=>setForm({...form,image_url:e.target.value})}/><ServiceImageUpload onUploaded={v=>setForm(x=>({...x,image_url:v}))}/>{form.image_url&&<div className="projectAdminImagePreview"><img src={form.image_url} alt="Aperçu du service"/></div>}</>}
