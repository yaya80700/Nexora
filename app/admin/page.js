...
export default function Admin(){
 const [items,setItems]=useState([]); const [tab,setTab]=useState("formation"); const [panel,setPanel]=useState("catalog"); const [editing,setEditing]=useState(null); const [adding,setAdding]=useState(false); const [form,setForm]=useState(empty); const [error,setError]=useState(""); const [loading,setLoading]=useState(true);
 ...
 function edit(item){setAdding(false);setEditing(item.id);setForm({...empty,...item,bullets:Array.isArray(item.bullets)?item.bullets:[]}); window.scrollTo({top:0,behavior:"smooth"});}
 function add(){setEditing(null);setAdding(true);setForm({...empty,type:tab,sort_order:visible.length+1});window.scrollTo({top:0,behavior:"smooth"});}
 async function save(e){e.preventDefault();setError("");const method=editing?"PATCH":"POST";const payload=editing?{...form,id:editing}:form;const r=await fetch("/api/admin/catalog",{method,headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const j=await r.json();if(!r.ok){setError(j.error||"Impossible d'enregistrer");return;}setEditing(null);setAdding(false);setForm(empty);load();}
 ...
 <section className="adminWrap">
  ...
  {panel==="editor"?<Editor />:panel==="users"?<AdminUsers embedded />:panel==="requests"?<AdminDemandes embedded />:<>
  <div className="adminTabs">{[["formation",GraduationCap],["service",Wrench],["site",Globe2]].map(([key,Icon])=><button key={key} className={tab===key?"active":""} onClick={()=>{setTab(key);setEditing(null);setAdding(false);setForm({...empty,type:key})}}><Icon size={16}/>{labels[key]}</button>)}</div>
