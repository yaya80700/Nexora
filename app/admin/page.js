...
import { Plus, Pencil, Trash2, Save, ShieldCheck, LogOut, GraduationCap, Wrench, Globe2, Sparkles, X, Upload, LoaderCircle } from "lucide-react";
...
export default function Admin(){
 ...
 function add(){if(tab==="subscription"&&subscriptions.length>=2){setError("Nexora est limité à 2 abonnements. Supprimez ou modifiez une formule existante avant d’en créer une nouvelle.");return;} setError("");setEditing(null);setAdding(true);setForm(tab==="subscription"?{...emptySubscription,sort_order:visible.length+1}:{...empty,type:tab,sort_order:visible.length+1});window.scrollTo({top:0,behavior:"smooth"});}
 ...
 <section className="adminWrap">
  ...
  <div className="adminHeader"><div><ShieldCheck size={17}/><strong>{labels[tab]}</strong><span>{visible.length} élément(s){tab==="subscription"?" · maximum 2":""}</span></div><button className="primary" onClick={add}><Plus size={16}/> Ajouter</button></div>
