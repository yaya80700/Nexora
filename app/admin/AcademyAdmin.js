...
export default function AcademyAdmin(){
  ...
  return <div className="academyAdmin">
    ...
    <div className="academyAdminList"><div className="academyAdminListHead"><div><span className="sectionTag">SUIVI PÉDAGOGIQUE</span><h3>Formations des utilisateurs</h3></div><span>{data.enrollments.length} parcours</span></div>
      {loading ? <p className="adminEmpty">Chargement…</p> : !data.enrollments.length ? (
        <div className="academyAdminEmpty"><LockKeyhole size={22}/><p>Aucune formation n'est encore activée.</p></div>
      ) : (
        data.enrollments.map((e) => {
          const user = userMap[e.user_id];
          const formation = formationMap[e.formation_slug];
          const total = Number(e.module_count) || 0;
          const current = Math.max(1, Math.min(total || 1, Number(e.current_module) || 1));
          const done = e.status === "completed" ? total : Math.max(0, current - 1);
          const pct = total ? Math.round((done / total) * 100) : 0;
          const currentLesson = formation?.bullets?.[current - 1] || `Module ${current}`;
          const last = current >= total;

          return (
            <article className="academyAdminItem" key={e.id}>
              <div className="academyAdminUser">
                <div className="academyAdminAvatar">{(user?.full_name || user?.email || "?").slice(0, 1).toUpperCase()}</div>
                <div><strong>{user?.full_name || "Utilisateur"}</strong><span>{user?.email || e.user_id}</span></div>
              </div>
              <div className="academyAdminCourse">
                <div><span>{formation?.category || "FORMATION"}</span><h4>{formation?.title || e.formation_title}</h4></div>
                <div className="academyAdminProgress">
                  <div><span>{done}/{total} modules</span><strong>{pct}%</strong></div>
                  <div><i style={{ width: `${pct}%` }} /></div>
                </div>
                <div className="academyAdminCurrent">
                  {e.status === "completed" ? (
                    <><CheckCircle2 size={17}/><div><span>FORMATION TERMINÉE</span><strong>Tous les modules sont validés.</strong></div></>
                  ) : (
                    <><PlayCircle size={17}/><div><span>MODULE ACTUEL</span><strong>{currentLesson}</strong></div></>
                  )}
                </div>
                <div className="academyAdminActions">
                  {e.status !== "completed" && (
                    <button className="primary" onClick={() => validate(e)} disabled={saving === e.id}>
                      {saving === e.id ? <RefreshCw className="spin" size={14}/> : <CheckCircle2 size={14}/>} {last ? "Valider et terminer" : "Valider le module"}
                    </button>
                  )}
                  <button className="adminDangerButton" onClick={() => remove(e)} disabled={saving === `delete:${e.id}`}><Trash2 size={14}/> Retirer</button>
                </div>
              </div>
            </article>
          );
