...
import {CheckCircle2,Clock3,Inbox,LoaderCircle,MessageCircle,Send,ShieldCheck, XCircle,LogOut,Paperclip,X} from "lucide-react";
...
function AttachmentList({attachments}){if(!Array.isArray(attachments)||!attachments.length)return null;return <div className="attachmentList">{attachments.map((file,i)=><a key={`${file.path}-${i}`} href={`/api/attachments?path=${encodeURIComponent(file.path)}`} target="_blank" rel="noreferrer"><Paperclip size={13}/><span>{file.name}</span></a>)}</div>}

export default function AdminDemandes({embedded=false}){
 const [requests,setRequests]=useState([]),[messages,setMessages]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[text,setText]=useState({}),[files,setFiles]=useState({}),[filter,setFilter]=useState("all"),[updating,setUpdating]=useState("");
 ...
 async function reply(id){if(!text[id]?.trim())return;setUpdating(id);const form=new FormData();form.append("request_id",id);form.append("message",text[id]);(files[id]||[]).forEach(file=>form.append("attachments",file));const r=await fetch("/api/admin/demandes",{method:"POST",body:form});const j=await r.json();if(!r.ok)setError(j.error||"Erreur");else{setText(x=>({...x,[id]:""}));setFiles(x=>({...x,[id]:[]}));await load()}setUpdating("")}
 ...
 const content=<>
  <div className="adminSectionHeading"><div><span className="sectionTag">DEMANDES CLIENTS</span><h2>Suivi des <span>demandes.</span></h2><p>Un suivi clair de chaque demande, de sa réception jusqu'à sa clôture. Les nouveaux messages sont récupérés automatiquement.</p></div></div>
  ...
  {loading?<div className="adminEmpty"><LoaderCircle className="spin"/> Chargement…</div>:filtered.length===0?<div className="adminEmpty"><MessageCircle size={30}/> Aucune demande dans ce statut.</div>:<div className="requestList adminRequests">{filtered.map(r=>{
   ...
   return <article className="requestCard requestAdminCard" key={r.id}>
    ...
    <div className="requestMessages">{msgs.map(m=><div className={`requestMessage ${m.sender_role}`} key={m.id}><b>{m.sender_role==="admin"?<><ShieldCheck size={12}/> Nexora</>:"Client"}</b><p>{m.message}</p><small>{new Date(m.created_at).toLocaleString("fr-FR")}</small><AttachmentList attachments={m.attachments}/></div>)}</div>
    {r.status!=="closed"?<div className="requestReply"><div className="replyComposer"><textarea rows="3" placeholder="Répondre au client…" value={text[r.id]||""} onChange={e=>setText(x=>({...x,[r.id]:e.target.value}))}/><label className="fileUploadButton small"><Paperclip size={14}/> Ajouter un fichier<input type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.zip,.png,.jpg,.jpeg,.webp,.gif" onChange={e=>{const selected=Array.from(e.target.files||[]);const current=files[r.id]||[];const next=[...current,...selected];if(next.length>3){setError("3 fichiers maximum par message.");return}if(next.some(f=>f.size>10*1024*1024)){setError("Chaque fichier doit faire 10 Mo maximum.");return}setError("");setFiles(x=>({...x,[r.id]:next}));e.target.value=""}}/></label>{(files[r.id]||[]).length>0&&<div className="selectedFiles">{(files[r.id]||[]).map((file,i)=><div key={`${file.name}-${i}`}><Paperclip size={12}/><span>{file.name}</span><button type="button" onClick={()=>setFiles(x=>({...x,[r.id]:(x[r.id]||[]).filter((_,n)=>n!==i)}))} aria-label={`Retirer ${file.name}`}><X size={13}/></button></div>)}</div>}</div><button className="primary" disabled={busy||!text[r.id]?.trim()} onClick={()=>reply(r.id)}>{busy?<LoaderCircle className="spin" size={15}/>:<Send size={15}/>} Envoyer la réponse</button></div>:<div className="requestClosedNote"><CheckCircle2 size={16}/><span>Cette demande est clôturée. Vous pouvez la rouvrir en cliquant sur un autre statut.</span></div>}
   </article>})}</div>}
 </>;
