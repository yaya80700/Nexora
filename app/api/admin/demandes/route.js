...
export async function POST(request){
  try{
    ...
    let attachments=[];try{attachments=await uploadRequestAttachments(supabase,files,userId,requestId,data.id);}catch(uploadError){await supabase.from("request_messages").delete().eq("id",data.id);return NextResponse.json({error:uploadError.message||"Impossible d'envoyer les fichiers."},{status:400});}
    if(attachments.length){const {error:updateError}=await supabase.from("request_messages").update({attachments}).eq("id",data.id);if(updateError)return NextResponse.json({error:updateError.message},{status:400});}
