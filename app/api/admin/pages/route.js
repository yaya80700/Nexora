...
async function admin() {
  ...
  if (claimsError || !claimsData?.claims?.sub) return { response: NextResponse.json({ error: "Non authentifié" }, { status: 401 }) };
  const { data: a } = await supabase.from("admin_users").select("user_id").eq("user_id", claimsData.claims.sub).maybeSingle();
  if (!a) return { response: NextResponse.json({ error: "Accès administrateur refusé" }, { status: 403 }) };
  return { supabase };
}
