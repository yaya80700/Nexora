...
async function getAdminClient() {
  ...
  if (claimsError || !claimsData?.claims) return { supabase:null, response:NextResponse.json({error:"Non authentifié"},{status:401}) };
  const { data: admin } = await supabase.from("admin_users").select("user_id").eq("user_id", claimsData.claims.sub).maybeSingle();
  if (!admin) return { supabase:null, response:NextResponse.json({error:"Accès administrateur refusé"},{status:403}) };
...
export async function PATCH(request) {
  ...
  const allowed = ["type","slug","title","name","full_name","category","level","icon","description","bullets","price","price_label","status","accent","url","active","sort_order"];
