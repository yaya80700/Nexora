...
export default async function Formations(){const formations=await getCatalog("formation");return <main><Header/>
...
<section className="formationGrid">{formations.map(f=><article className="formationCard" key={f.slug}><div className="formationIcon">{f.icon}</div><div className="cardMeta">{f.category}<span>{f.level}</span></div><h2>{f.title}</h2><p>{f.description||f.desc}</p><div className="formationInfo"><span><Clock3 size={14}/> Parcours progressif</span><span><CheckCircle2 size={14}/> Projet pratique</span></div><ul>{(f.bullets||[]).slice(0,3).map(x=><li key={x}><CheckCircle2 size={15}/>{x}</li>)}</ul><div className="priceRow"><strong>{Number(f.price).toFixed(2).replace(".",",")} €</strong><Link className="smallBtn" href={`/formations/${f.slug}`}>Voir le programme <ArrowRight size={16}/></Link></div></article>)}</section>
