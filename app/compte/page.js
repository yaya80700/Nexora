...
export default async function Compte() {
  ...
  const completedModules = academyList.reduce((sum, x) => sum + Math.max(0, Math.min(Number(x.module_count || 0), Number(x.current_module || 1) - 1)), 0);
    ...
    <section className="subPage accountPage">
      <div className="subHero accountHero">
        ...
        <div className="accountStatsGrid">
          ...
          <Stat icon={<Clock3 />} value={unread} label="À consulter" hint={unread ? "Réponse(s) de Nexora" : "Tout est à jour"} />
        </div>
        ...
        <section className="accountQuickLinks"><Quick href="/profil" icon={<UserRound />} title="Mon profil" text="Informations du compte" /><Quick href="/demandes" icon={<MessageSquare />} title="Mes demandes" text="Suivi et conversations" badge={unread} /><Quick href="/academy" icon={<BookOpen />} title="Nexora Academy" text="Formations et progression" /><Quick href="/contact" icon={<FileText />} title="Nouvelle demande" text="Parler à l'équipe Nexora" /></section>
