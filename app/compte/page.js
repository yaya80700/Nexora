...
import { ArrowRight, ArrowUpRight, BookOpen, CheckCircle2, Clock3, FileText, LogOut, MessageSquare, Sparkles, UserRound, Bell } from "lucide-react";
...
export default async function Compte() {
  ...
  const [{ data: requests }, { data: enrollments }, { data: accountSubscription }, { data: admin }, { data: notificationRows }] = await Promise.all([
    ...
    supabase.from("user_subscriptions").select("subscription_slug,subscription_name,assigned_at,starts_at,expires_at,status,renewed_at,renewal_count").eq("user_id", user.id).maybeSingle(),
    supabase.from("admin_users").select("user_id").eq("user_id", user.id).maybeSingle(),
    supabase.from("notifications").select("id").eq("user_id", user.id).is("read_at", null).limit(99),
  ]);
    ...
    <section className="subPage accountPage">
      <div className="subHero accountHero">
        ...
        <div className="accountStatsGrid">
          ...
          <Stat icon={<BookOpen />} value={academyList.length} label="Formations" hint={completedFormations.length ? `${completedFormations.length} terminée(s)` : activeFormations.length ? `${activeFormations.length} en cours` : "Aucun parcours"} />
          <Stat icon={<Sparkles />} value={accountSubscription ? (accountSubscription.status === "expired" || (accountSubscription.expires_at && new Date(accountSubscription.expires_at).getTime() <= Date.now()) ? "Expiré" : "Actif") : "—"} label="Abonnement" hint={accountSubscription ? `Attribué le ${formatDate(accountSubscription.assigned_at)}${accountSubscription.expires_at ? ` · expire le ${formatDate(accountSubscription.expires_at)}` : ""}` : "Aucune formule active"} />
          <Stat icon={<Clock3 />} value={unread} label="Réponses" hint={unread ? "Dernières réponses de Nexora" : "Aucune réponse récente"} />
        </div>

        <div className="accountProgressSummary">
          <div className="accountProgressSummaryHead">
            <div><span className="sectionTag">PROGRESSION GLOBALE</span><strong>{globalProgress}%</strong></div>
            <Link href="/academy">Voir Academy <ArrowUpRight size={13} /></Link>
          </div>
          <div className="accountProgressLarge"><i style={{ width: `${globalProgress}%` }} /></div>
          <p>{academyList.length ? `${completedModules} module(s) validé(s) sur ${totalModules}.` : "Aucune formation active pour le moment."}</p>
        </div>
        ...
        <div className="accountMainGrid">
          ...
          <section className="accountBox accountOverviewBox">
            ...
            <div className="accountOverviewList">
              ...
              <Overview icon={<BookOpen />} label="Formations terminées" value={completedFormations.length} href="/academy" />
              <Overview icon={<BookOpen />} label="Modules validés" value={completedModules + "/" + totalModules} href="/academy" />
        ...
        <div className="accountBottomGrid">
          ...
          <section className="accountBox">
            ...
            <div className={`accountPlan ${subscriptionClass}`} style={subscriptionStyle}><div className="accountPlanIcon"><Sparkles size={19} /></div><div><span>FORMULE ACTUELLE</span><strong>{accountSubscription?.subscription_name || "Aucun abonnement"}</strong><p>{accountSubscription ? `Attribué le ${formatDate(accountSubscription.assigned_at)}${accountSubscription.expires_at ? ` · ${accountSubscription.status === "expired" ? "expiré le" : "expire le"} ${formatDate(accountSubscription.expires_at)}` : ""}` : "Choisissez une formule adaptée à vos besoins."}</p></div></div>
        ...
        <section className="accountQuickLinks"><Quick href="/profil" icon={<UserRound />} title="Mon profil" text="Informations du compte" /><Quick href="/parametres" icon={<Sparkles />} title="Paramètres" text="Sécurité et préférences" /><Quick href="/demandes" icon={<MessageSquare />} title="Mes demandes" text="Suivi et conversations" badge={unread} /> <Quick href="/notifications" icon={<Bell />} title="Notifications" text="Vos mises à jour Nexora" badge={notificationUnread} /><Quick href="/academy" icon={<BookOpen />} title="Nexora Academy" text="Formations et progression" /><Quick href="/contact" icon={<FileText />} title="Nouvelle demande" text="Parler à l'équipe Nexora" /></section>
