...
export default async function About(){
  ...
  return <main><Header/>
    ...
    <section className="tipJarSection">
      ...
      <div className="tipJarContent">
        <span className="sectionTag">SOUTENIR NEXORA</span>
        <h2>Un petit coup de pouce pour <span>faire grandir Nexora.</span></h2>
        <p>Si Nexora, ses projets ou ses ressources vous sont utiles, vous pouvez soutenir librement son développement. Chaque contribution aide à faire évoluer la plateforme.</p>
        {process.env.NEXT_PUBLIC_TIP_URL ? (
          <a href={process.env.NEXT_PUBLIC_TIP_URL} target="_blank" rel="noopener noreferrer" className="primary tipJarButton">
            <Heart size={17} /> Laisser un pourboire <ArrowUpRight size={17}/>
          </a>
        ) : (
          <span className="tipJarComingSoon"><Heart size={16}/> Lien de soutien bientôt disponible</span>
        )}
      </div>
    </section>
