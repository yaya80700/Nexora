# Nexora V1.6.59 — Audit production & sécurité

Base : Nexora V1.6.58 — adaptation mobile de la page d’accueil.

## V1.6.59 — Audit production & sécurité
- Next.js mis à jour en `15.5.26`, correctif de sécurité de la branche 15.5.
- Protection du callback OAuth contre les redirections externes/ambiguës.
- API `/api/site-content` limitée aux pages publiques connues et aux pages publiées.
- Configuration Vercel mise à jour avec `NEXT_PUBLIC_SITE_URL` et la version courante.
...
## V1.6.47 — Préparation production
...
- Version du projet mise à jour en `1.6.47`.
- Aucun fichier `.js` renommé en `.mjs` : `next.config.js` reste en `.js`.

## V1.6.51 — Audit accès privés & version de production
- Version centralisée dans `lib/nexora/version.js`.
- Endpoint `/api/health` aligné sur la version réelle du projet.
- Protection middleware renforcée pour `/notifications`, `/parametres` et `/academy`.
- Les espaces privés sont exclus des robots d’indexation.
