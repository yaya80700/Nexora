...
Nexora est une plateforme numérique moderne qui rassemble formations, services, projets et accompagnement. La V0.8 corrige la configuration des clés Supabase et passe d'une simple interface de démonstration à une base d'authentification réelle prête pour la production avec Supabase.

## Nouveautés V0.8.2

- Middleware Supabase renforcé pour éviter `MIDDLEWARE_INVOCATION_FAILED`.
- Le middleware ne s’exécute plus sur tout le site : il protège uniquement `/compte`.
- Si les variables Supabase manquent ou sont invalides, Nexora redirige vers `/connexion` au lieu de provoquer une erreur 500.
- Une seule vérification d’authentification est effectuée dans le middleware.
- Les cookies Supabase SSR continuent d’être synchronisés avec `getAll` / `setAll`.
...
2. Dans **Project Settings → API**, récupère l'URL du projet et la **Publishable key** (`sb_publishable_...`).
