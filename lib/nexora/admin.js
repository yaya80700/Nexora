...
const legacyPermissions={owner:["all"],admin:["users_read","users_manage","staff_manage","academy_manage","catalog","editor","requests","requests_manage","site_pages"],editor:["editor","catalog","site_pages"],support:["users_read","requests"]};
...
export async function getAdminContext(){
 ...
 const permissions={};
 // Les rôles système conservent leurs accès de base, même après l'activation
 // du système multi-rôles. Les rôles personnalisés ajoutent ensuite leurs accès.
 const legacy=legacyPermissions[staff.role]||[];
 for(const p of legacy){if(p==="all")permissions.all=true;else permissions[p]=true}
 if(staff.permissions && typeof staff.permissions === "object" && !Array.isArray(staff.permissions)){
   for(const [p,v] of Object.entries(staff.permissions)){if(v)permissions[p]=true}
 }
 for(const r of roleList){
   if(Array.isArray(r.permissions))
     for(const p of r.permissions){if(p==="all")permissions.all=true;else permissions[p]=true}
 }
