...
export async function uploadRequestAttachments(supabase, files, userId, requestId, messageId = null) {
  ...
  for (const file of list) {
    ...
    const path = `requests/${userId}/${requestId}/${messageId ? `messages/${messageId}/` : "initial/"}${crypto.randomUUID()}.${ext}`;
    const { error } = await supabase.storage.from("nexora-attachments").upload(path, file, { contentType: file.type || "application/octet-stream", upsert: false });
